...
The function `replicate` repeatedly evaluates an expression. This is useful for simulations, which we will see in the example of the Birthday Problem. The following is an example of samping 10 times from an Exponential distribution 100 times. 
...
Simulate the birthday problem with 25 people. The funcions `runif`, `round`, and `replicate` may be helpful.
...
```{r}
x <- round(runif(25, 1, 365))
```

Find a way to determine if someone has a matching birthday. Make this process into a function that returns TRUE and FALSE (or 1 and 0).
...
Replicate that simulation an arbitrarily large number of times (10,000 and above) and store the result (it will automatically be a list). Try simplifying into a vector for convenience.

```{r}
bday_rep <- replicate(10000, bday(25))
```

Take the mean of the vector to get the probability of a birthday match. 

```{r}
mean(bday_rep)
```

Here's an alternate solution that uses `duplicated()`. 
